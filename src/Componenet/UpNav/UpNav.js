      ...
      <div className="bg-zinc-800 rounded-lg mt-2 overflow-hidden transition-all duration-300">
        {subItems[itemName]?.map((subItem) => (
          <div
            ...
            onClick={(e) => {
              e.stopPropagation();
              // Navigate to the correct path based on the item name
              if (itemName === "موزیک") {
                navigate(`/product/${subItem.key}`);
              } else if (itemName === "ویدیو") {
                navigate(`/videoPlayer/${subItem.key}`);
              } else if (itemName === "گرافیک") {
                navigate(`/Graphicplayer/${subItem.key}`);
              }
              setMenuOpen(false);
            }}
          >
